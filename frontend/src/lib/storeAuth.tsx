 ...
 * slots (handled inside storeApi: in-memory access token + `arya_store_refresh`
 * in sessionStorage). The platform admin/applicant identity in `auth.tsx` is
 * untouched — the two can be authenticated independently in the same tab.
 *
 * Mirrors the hydration pattern in `auth.tsx`: on mount, if there is no
 * in-memory access token, attempt a silent refresh from the persisted refresh
 * token; a failed refresh clears any stale credential. Once a token is present
 * (or freshly minted via login/register), `meCustomer()` hydrates the profile.
 */
...
export function StoreAuthProvider({ children }: { children: ReactNode }) {
  ...
  const hydrate = useCallback(async () => {
    try {
      ...
      // reload, try a silent refresh from the persisted `arya_store_refresh`.
  ...
  const login = useCallback(
    async (input: StoreLoginInput): Promise<StoreCustomer> => {
      setLoading(true);
      try {
        // storeApi.loginCustomer persists the token pair internally (own slot +
