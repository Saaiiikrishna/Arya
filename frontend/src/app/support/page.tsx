...
export default function SupportPage() {
  ...
  useEffect(() => {
    api.getSupportStats().then(setStats).catch(console.error);
  }, []);

  const initiateSupport = async (e: React.FormEvent) => {
    e.preventDefault();
    if (form.amount < 100) {
    ...
    try {
      const orderData = await api.createSupportOrder(form);
      ...
      <div className="max-w-[1200px] mx-auto py-20 px-6 animate-fade-in">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-ink/70 max-w-2xl mx-auto">
            100% of your contributions go directly into funding infrastructure, tools, and resources for the Aryavartham builder pipeline.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="flex flex-col gap-8 flex-1">
            ...
            <div className="border border-hairline p-8 bg-white">
              <h3 className="font-serif text-xl font-bold mb-4">Why Support us?</h3>
              <ul className="space-y-4 text-ink/80 text-sm">
                ...
                <li className="flex gap-3">
                  <span className="text-forest">•</span>
                  <span>Identified contributors receive priority access to demo days.</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="bg-white border border-hairline p-8 lg:p-10 shadow-sm">
            {paymentStatus === 'SUCCESS' ? (
              ...
              <form onSubmit={initiateSupport} className="flex flex-col gap-6">
                <div>
