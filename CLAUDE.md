...
- **Refresh-token transport is cross-domain-aware** — the backend reads the refresh token from EITHER an HttpOnly cookie (`arya_refresh`, `path=/api`, `Secure`+`SameSite=None` prod / `Lax` dev) OR the request body, and sets the cookie on every issue/rotate (helpers in `backend/src/modules/auth/refresh-cookie.ts`). In the CURRENT prod topology the SPA (`aryavartham.com`) and API (`*.azurecontainerapps.io`) are DIFFERENT registrable domains, so that cookie is third-party and browsers block it — therefore the frontend persists the refresh token in `sessionStorage` (`arya_refresh`) and sends it in the body (the reliable transport), while still sending `credentials:'include'`. **Do NOT make the frontend rely on the cookie alone** until the API is served same-site (e.g. `api.aryavartham.com`); doing so logs users out on every reload. Cookie-auth refresh/logout are CSRF-guarded by an Origin allow-list (`assertTrustedOrigin`). (Issue #3 — partially deferred pending same-site API)
...
- **Store customer auth is a SEPARATE identity** — dedicated `jwt-customer` Passport strategy + `CustomerJwtGuard` (never falls through to the platform/admin validator). Customer passwords bcrypt; customer refresh tokens SHA-256-hashed + rotated in `refresh_tokens`. The customer refresh token uses the SAME cross-domain-aware transport as the platform (its OWN cookie `arya_store_refresh` set by the server, but persisted in `sessionStorage` + sent in the body since the cookie is third-party in the current topology) and the SAME RFC 9700 family reuse detection (rotated tokens revoked-not-deleted, CAS, family revoke on replay past a 10s leeway, unique `jti`). Cookie helpers are name-parameterized in `auth/refresh-cookie.ts`. Guest carts/orders are authorized by SHA-256 token-hash match (`GuestCartGuard`/`GuestOrderGuard`) — a bare UUID is never sufficient. Product/tab-section content is also untrusted JSON → sandbox-render only.
