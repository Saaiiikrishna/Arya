...
- **Store customer auth is a SEPARATE identity** — dedicated `jwt-customer` Passport strategy + `CustomerJwtGuard` (never falls through to the platform/admin validator). Customer passwords bcrypt; customer refresh tokens SHA-256-hashed + rotated in `refresh_tokens`. The customer refresh token is delivered via its OWN HttpOnly cookie (`arya_store_refresh`, `path=/api`, `Secure`+`SameSite=None` prod / `Lax` dev — never JS-readable storage; frontend keeps only an `arya_store_has_session` hint) and uses the SAME RFC 9700 family reuse detection as the platform (rotated tokens revoked-not-deleted, CAS, family revoke on replay past a 10s leeway, unique `jti`). Cookie helpers are name-parameterized in `auth/refresh-cookie.ts`. Guest carts/orders are authorized by SHA-256 token-hash match (`GuestCartGuard`/`GuestOrderGuard`) — a bare UUID is never sufficient. Product/tab-section content is also untrusted JSON → sandbox-render only.
