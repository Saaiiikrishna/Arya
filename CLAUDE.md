...
Built: auth, applicant, batch, team, eligibility, question, document, email, project, sprint, ledger, payment, donation, equity, matching, investor (+ pitch events), training, analytics, chat, election, announcement, referral, interview, mentor, cofounder, documentary, whatsapp (shell only), settings
...
All backend calls go through `src/lib/api.ts` (Axios wrapper). Never use `fetch` directly — the wrapper attaches the auth header and base URL. Auth token handling lives in `src/lib/auth.tsx`.
...
- **`pgcrypto` extension removed** (`888c39a`) — migrations must not reference it
- **S3 bucket name:** `arya-documents` (region: `ap-south-1`)
